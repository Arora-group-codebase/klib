...
__all__ = ['preresnet20', 'preresnet32', 'preresnet56']


class PreResNet(nn.Module):


    def __init__(self, depth, num_classes=10, block_name='BasicBlock', norm_layer=nn.BatchNorm2d):
        super().__init__()
        ...
        self.final_layer = nn.Linear(64 * block.expansion, num_classes)
...
def _preresnet(depth, args):
    norm_layer = lambda w: klib.train_utils.get_norm2d(w, args)
    return KModel, PreResNet(depth=depth, norm_layer=norm_layer)


def preresnet20(args):
    return _preresnet(20, args)


def preresnet32(args):
    return _preresnet(32, args)


def preresnet56(args):
